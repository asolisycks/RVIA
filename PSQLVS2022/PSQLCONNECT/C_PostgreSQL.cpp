...
bool C_PostgreSQL::openConnection(ES_PostgresConnection pg_connection)
{
	...
	sprintf(cStringConnection, "dbname=%s user=%s password=%s hostaddr=%s port=5432", pg_connection.database, pg_connection.user, pg_connection.password, pg_connection.IPAddress);
