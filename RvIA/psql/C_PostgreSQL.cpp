...
std::string C_PostgreSQL::sanitize(const std::string& input) 
{
	char* output = new char[input.size() * 2 + 1];
	PQescapeStringConn(pgConnection, output, input.c_str(), input.size(), nullptr);
